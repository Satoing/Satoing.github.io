...
(function(global, factory) { typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() : typeof define === 'function' && define.amd ? define(factory) : (global = global || self, global.ckplayer = factory()) }(this, function() {
    ...
    function ckplayerEmbed(videoObj) {
        ...
        var adFrontSetTime = null,
            adPauseSetTime = null;
        var closeTipFun = null;
        var closeTipMouseOut = null;
        var waitingMessage = true;
        var hidePreviewSetTime = null;
        var pSliderMouseDown = false;
        var playbackTime = 0;
        var loadedmetadataNum = 0;
        ...
        var into = function(obj) {
                if (isUndefined(obj)) { player.add = player.into = into; return player }
                language = decodeURIString(language);
                if (!isUndefined(window.ckplayerLanguage)) { language = mergeObj(language, window.ckplayerLanguage) }
                eventTarget();
                documentHidden(function(state) { eventTarget('visibilityState', state) });
                app = getApp();
                varsTemp = {};
                for (var key in videoObjectDefault) { varsTemp[key] = videoObjectDefault[key] }
                if (valType(obj) == 'string' && (obj.substr(0, 8) == 'website:' || obj.substr(0, 4) == 'url:')) {
                    var ajaxUrl = '';
                    if (obj.substr(0, 8) == 'website:') { ajaxUrl = obj.substring(8) }
                    if (obj.substr(0, 4) == 'url:') { ajaxUrl = obj.substring(4) }
                    ajax({ url: ajaxUrl, success: function(data) { if (data) { return into(data) } else { showWindowsError(language['error']['ajax'] + ',url:' + ajaxUrl); return null } } })
                } else if (valType(obj) == 'object') { varsTemp = standardization(varsTemp, obj); if (!isUndefined(obj['container']) && obj['container']) { CT = $(obj['container']); if (CT) { return loadLanguage(obj) } else { return bodyReady(obj) } } else { return bodyReady(obj) } } else { showWindowsError(language['error']['noVideoContainer']); return null }
            },
            bodyReady = function(obj) { documentReady(function() { if (!isUndefined(obj['container'])) { CT = $(obj['container']); if (CT) { return loadLanguage(obj) } else { showWindowsError(language['error']['noVideoContainer']); return null } } else { return ajaxWebsite(obj) } }); return null },
            ajaxWebsite = function(obj) {
                if (valType(obj['video']) == 'string' && (obj['video'].substr(0, 8) == 'website:' || obj['video'].substr(0, 4) == 'url:')) {
                    ...
                    var ajaxUrl = '';
                    if (obj['video'].substr(0, 8) == 'website:') { ajaxUrl = obj['video'].substring(8) }
                    if (obj['video'].substr(0, 4) == 'url:') { ajaxUrl = obj['video'].substring(4) }
                    ajax({ url: ajaxUrl, success: function(data) { if (data) { return into(data) } else { showWindowsError(language['error']['ajax'] + ',url:' + ajaxUrl); return null } } })
                } else { showWindowsError(language['error']['noVideoContainer']); return null }
            },
            loadLanguage = function(obj) {
                ...
                if (vars['language']) {
                    ...
                    loadJs(path, function() { if (!isUndefined(window['ckplayerLanguage'])) { language = mergeObj(language, window['ckplayerLanguage']) } return embed(obj) })
                } else { return embed(obj) }
            },
            embed = function(obj) {
                mutedState = vars['volume'] > 0 ? false : true;
                if (!isUndefined(CT) && CT != null) { if (valType(CT) == 'htmlarray') { CT = CT.eq(0) } } else { return null }
                calculationFps();
                CT.htm('');
                ...
                if (valType(vars['video']) == 'string' && ((vars['video'].substr(0, 1) == '.' && vars['video'].indexOf('/') == -1) || vars['video'].substr(0, 1) == '#')) { video = $(vars['video']) }
                if (!video) {
                    video = createlVideo();
                ...
                if (vars['autoplay']) {
                    ...
                    paused = false
                } else { video.attr('preload', 'metadata') }
                CT.loop = vars['loop'];
                if (CT.loop) { video.attr('loop', 'loop') }
                if (vars['controls']) { video.controls = true } else { video.controls = false }
                try {
                    ...
                    if (vars['crossOrigin']) {
                        video.useCORS = true;
                        video.crossOrigin = vars['crossOrigin']
                    }
                } catch (event) {}
                ...
                CT.theatre = false;
                loadFace();
                if (vars['rotate'] > 0) { player.rotate(vars['rotate']) }
                if (vars['zoom'] > 0) { player.zoom(vars['zoom']) }
                ...
                addAllListener();
                if (!isUndefined(vars['loaded'])) { if (valType(vars['loaded']) == 'function') { try { vars['loaded'](player) } catch (event) {} } else if (valType(vars['loaded']) == 'string') { try { eval(vars['loaded'] + '(player)') } catch (event) {} } }
                changeVideo(vars['video']);
                return player
            },
            changeVideo = function(vstr) {
                var i = 0;
                if (video.attr('src') || video.htm()) { player.pause() }
                if (video.attr('src')) {
                    video.attr('src', '');
                    video.removeAttr('src')
                }
                if (!isUndefined(video.textTracks) && video.textTracks.length > 0) { for (i = video.find('track').length - 1; i > -1; i--) { video.find('track').eq(i).remove() } }
                video.htm('');
                if (!isUndefined(vars['ad'])) { ad = vars['ad'] }
                var source = '';
                if (valType(vstr) == 'string') {
                    if (vstr.substr(0, 8) == 'website:' || vstr.substr(0, 4) == 'url:') {
                        var ajaxUrl = '';
                        if (vstr.substr(0, 8) == 'website:') { ajaxUrl = vstr.substring(8) }
                        if (vstr.substr(0, 4) == 'url:') { ajaxUrl = vstr.substring(4) }
                        ajax({
                            url: ajaxUrl,
                        ...
                        if (vars['plug'] && !canPlay(vstr)) { plugPlayer(vstr) } else { video.attr('src', vstr) }
                        loadTrack()
                    }
                } else if (valType(vstr) == 'array') {
                    for (i = 0; i < vstr.length; i++) { if (valType(vstr[i]) == 'array') { if (vars['plug'] && !canPlay(vstr[i][0])) { plugPlayer(vstr[i][0]) } else { var type = ' type="' + vstr[i][1] + '"'; if (vstr[i].length > 1) { source += '<source src="' + decodeURIComponent(vstr[i][0]) + '"' + type + '>' } } } }
                    if (source) { video.htm(source) }
                    loadTrack()
                } else if (valType(vstr) == 'object') {
                    if (vars['plug']) { plugPlayer(vstr) } else {
                ...
                if (!isUndefined(video.find('source')) && video.find('source').length > 0) { video.find('source').eq(video.find('source').length - 1).addListener('error', videoHandler.error) }
                checkBar();
                loadLogo();
                if (isChangeDef) { checkDefinition() }
                if (vars['cookie']) { if (valType(vars['cookie']) == 'array' && vars['cookie'].length >= 2) { if (vars['cookie'][0] && valType(vars['cookie'][0]) == 'string') { cookieName = vars['cookie'][0] } if (vars['cookie'][1] && valType(vars['cookie'][1]) == 'number') { cookieTime = vars['cookie'][1] } } if (valType(vars['cookie']) == 'object' && !isUndefined(vars['cookie']['name'])) { cookieName = vars['cookie']['name']; if (!isUndefined(vars['cookie']['hour']) && valType(vars['cookie']['hour']) == 'number') { cookieTime = vars['cookie']['hour'] } } if (valType(vars['cookie']) == 'string') { cookieName = vars['cookie'] } if (cookieName) { cookieName = cookieName.replace(/[ ]*,[ ]*|[ ]+/g, '').replace(/[ ]*;[ ]*|[ ]+/g, '') } }
                if (!isUndefined(vars['seek']) && !seekTime) { if (valType(vars['seek']) == 'number' && vars['seek'] > 0) { seekTime = vars['seek'] } if (valType(vars['seek']) == 'string' && vars['seek'] == 'cookie' && cookieName) { var cke = player.cookie(cookieName); if (cke) { seekTime = cke['time'] } } }
            },
            plugPlayer = function(url) {
                switch (vars['plug']) {
                    ...
                    case 'dash.js':
                        ...
                        break;
                    default:
                        if (valType(vars['plug']) == 'function') { vars['plug'](video, url) } else {
                            CT.error = { code: 10, message: language['error']['emptied'] };
                            eventTarget('error', CT.error);
                            showError()
                        }
                ...
                loadJs(path, function() {
                    if (!isUndefined(Hls) && Hls.isSupported()) {
                        ...
                        hls.on(Hls.Events.ERROR, function(event, data) {
                            ...
                            if (!isUndefined(data.response)) { if (!isUndefined(data.response.code)) { code = data.response.code } if (!isUndefined(data.response.text) && data.response.text) { message = data.response.text } else { if (!isUndefined(event)) { message = event } } }
                            CT.error = { code: code, message: message };
                            eventTarget('error', CT.error);
                            if (fatal) { showError() }
                        })
                    } else if (canPlay(url)) { video.attr('src', url) }
                })
            },
            flvPlayer = function(url) {
                ...
                loadJs(path, function() {
                    if (typeof(flvjs) != 'undefined' && !isUndefined(flvjs) && flvjs.isSupported()) {
                        var config = { type: 'flv', url: url };
                        if (valType(url) == 'object') { config = url }
                        if (vars['live']) { config['isLive'] = true }
                        if (pugPlayer) {
                            pugPlayer.pause();
                            pugPlayer.unload();
                        ...
                        pugPlayer.load();
                        pugPlayer.on(flvjs.Events.METADATA_ARRIVED, function(res) { videoHandler.loadedMetaData() });
            ...
            mpegtsPlayer = function(url) {
                ...
                loadJs(path, function() {
                    if (typeof(mpegts) != 'undefined' && !isUndefined(mpegts) && mpegts.getFeatureList().mseLivePlayback) {
                        ...
                        pugPlayer.attachMediaElement(video);
                        pugPlayer.load();
                        pugPlayer.on(mpegts.Events.METADATA_ARRIVED, function(res) { videoHandler.loadedMetaData() });
            ...
            canPlay = function(vStr, num) {
                var vTypeArr = [{ vidType: 'video/ogg', codType: 'theora, vorbis' }, { vidType: 'video/mp4', codType: 'avc1.4D401E, mp4a.40.2' }, { vidType: 'video/mp4', codType: 'avc1' }, { vidType: 'video/mp4', codType: 'hevc' }, { vidType: 'video/webm', codType: 'vp8.0, vorbis' }, { vidType: 'video/webm', codType: 'vp9' }, { vidType: '', codType: 'application/x-mpegURL' }, { vidType: '', codType: 'application/vnd.apple.mpegurl' }];
                var vType = { ogg: vTypeArr[0], mp4: [vTypeArr[1], vTypeArr[2], vTypeArr[3]], webm: [vTypeArr[4], vTypeArr[5]], m3u8: [vTypeArr[6], vTypeArr[7]] };
                var arr = [];
                var k = '';
                var supportType = function(obj) { var vType = obj['vidType']; var str = ''; if (vType) { str = vType + '; codecs="' + obj['codType'] + '"' } else { str = obj['codType'] } var sup = video.canPlayType(str); if (sup == '') { sup = 'no' } return sup };
                var getExtension = function(filepath) { return filepath.replace(/.+\./, '') };
                if (isUndefined(num)) { if (valType(vStr) == 'string') { if (vars['type']) { for (k in vTypeArr) { if (vTypeArr[k]['vidType'] == vars['type']) { arr.push(vTypeArr[k]) } } } else { if (getExtension(vStr) && getExtension(vStr) in vType) { var temp = vType[getExtension(vStr)]; if (valType(temp) == 'array') { for (var i = 0; i < temp.length; i++) { arr.push(temp[i]) } } else { arr.push(temp) } } } } } else { arr.push(vTypeArr[num]) }
                var is = false;
                if (arr.length > 0) { for (var i = 0; i < arr.length; i++) { if (supportType(arr[i]) != 'no') { is = true } } }
                if (!is && !vars['plug']) {
            ...
            loadFace = function() {
                ...
                C['bar']['nextEpisode'].click(nextClick).mouseover(showNextEpisode).mouseout(function() { if (!isUndefined(C['next'])) { player.closeLayer(C['next']) } });
                if (isUndefined(vars['next'])) { C['bar']['nextEpisode'].hide() }
                var timeDefault = formatSeconds(player.time()) + '/' + formatSeconds(duration);
                if (vars['live']) { timeDefault = language['live'] }
                ...
                C['bar']['backLive'].mouseover(function() { tip(this, language['backLive']) }).click(function() { eventTarget('backLive') });
                C['bar']['backLive'].hide();
                ...
                if (!vars['webFull']) { C['bar']['webFullAndExit'].hide() }
                ...
                if (!vars['theatre']) { C['bar']['theatreAndExit'].hide() }
                ...
                if (app == 'iphone') { C['loading'].hide() }
                changeTopTime();
                CK.mouseout(function() { if (!paused) { hideBar() } }).mouseover(function() {
                    if (!C['bar'].hasClass('ck-bar-out')) {
                        C['bar'].removeClass('ck-bar-out');
                        CM.removeClass('ck-nocursor')
                    }
                    if (C['rightBar'].css('display') == 'block') { C['rightBar'].removeClass('ck-right-bar-hide') }
                    if (C['topBar'].css('display') == 'block') { C['topBar'].removeClass('ck-top-bar-hide') }
                }).mousemove(function() {
                    if (C['bar'].hasClass('ck-bar-out')) {
                        C['bar'].removeClass('ck-bar-out');
            ...
            addAllListener = function() {
                video.addListener('stalled', videoHandler.stalled);
                video.addListener('suspend', videoHandler.suspend);
                video.addListener('loadeddata', videoHandler.loadedData);
                video.addListener('loadstart', videoHandler.loadStart);
                video.addListener('loadedmetadata', videoHandler.loadedMetaData);
                video.addListener('canplay', videoHandler.canPlay);
                video.addListener('timeupdate', videoHandler.timeUpDate);
                video.addListener('seeking', videoHandler.seeking);
                video.addListener('seeked', videoHandler.seeked);
                video.addListener('waiting', videoHandler.waiting);
                video.addListener('play', videoHandler.play);
                video.addListener('pause', videoHandler.pause);
                video.addListener('volumechange', videoHandler.volumeChange);
                video.addListener('ended', videoHandler.ended);
                video.addListener('error', videoHandler.error);
                if (!isUndefined(video.emptied)) { video.addListener('emptied', videoHandler.emptied) }
                CV.singleClick(function() { if (!isDrag) { player.playOrPause() } else { isDrag = false } });
                CM.doubleClick(player.fullOrExit);
                $(document).addListener('keydown', videoHandler.keydown);
                addListener(window, 'resize', videoHandler.resize);
                if (!isUndefined(vars['smallWindows'])) { if (valType(vars['smallWindows']) == 'boolean' && vars['smallWindows']) { addListener(window, 'scroll', windowScroll) } }
            ...
            eventTarget = function(name, obj) {
                try {
                    if (EventTarget != null) {
                        ...
                        EventTarget = function() { this.listeners = {} };
                        ...
                        EventTarget.prototype.dispatchEvent = function(type, obj) { if (!(type in this.listeners)) { return } var stack = this.listeners[type]; for (var i = 0, l = stack.length; i < l; i++) { stack[i].call(this, obj) } };
            ...
            changeCookie = function(time) {
                ...
                time = time * 0.01;
                if (!cookieTime) { cookieTime = 365 * 24 }
                if (!cStr) { arr.push([cookieName, time, cTime + cookieTime * 3600]) } else {
            ...
            loadTrack = function() {
                if (isUndefined(video.textTracks)) { return }
                var i = 0;
                if (!isUndefined(vars['track']) && valType(vars['track']) == 'array' && !video.find('track')) {
                    ...
                    for (i = 0; i < arr.length; i++) {
                        var def = '';
                        if (!def && !isUndefined(arr[i]['default']) && arr[i]['default']) { def = ' default' }
            ...
            loadedMetaData = function() {
                eventTarget('loadedMetaData', { width: CK.getWidth(), height: CK.getHeight(), videoWidth: CT.videoWidth, videoHeight: CT.videoHeight, duration: duration, volume: CT.volume });
                if ('error' in C) { C['error'].hide() }
                ...
                documentHidden(function(state) {
                    if (vars['documentFocusPause']) {
                        if (state == 'show') { if (!focusPause && paused) { player.play() } } else {
                            focusPause = paused;
                            player.pause()
                        }
                    }
        ...
        var videoHandler = {
            loadStart: function() {
                if (!vars['controls'] && app != 'iphone') { C['loading'].show() }
                C['centerPlay'].hide();
            ...
            loadedData: function() { eventTarget('loadeddata') },
            loadedMetaData: function() {
                duration = this.duration;
                if (!isUndefined(this.duration) && vars['duration']) { duration = vars['duration'] }
                if (!vars['live']) {
                ...
                C['loading'].hide();
                if (vars['poster']) { video.attr('poster', vars['poster']) }
                ...
                if (CT.videoWidth || CT.videoHeight || duration) { loadedMetaData() }
                var len = 0;
                if (!isUndefined(this.buffered)) { len = this.buffered.length }
                if (len > 0) { changeLoad() }
                if (!vars['live']) { changePreview(vars['preview']) } else { changePreview() }
                if (!isUndefined(vars['prompt']) && valType(vars['prompt']) == 'array' && !vars['live']) { changePrompt() } else { deletePrompt() }
                if (isChangeDef) {
                    checkDefinition();
                    isChangeDef = false
                }
                ...
                loadedmetadataNum++;
                if (!isUndefined(ad) && !isUndefined(ad['frontPlay']) && ad['frontPlay'] && loadedmetadataNum > 1) {
                    player.play();
                    calculationAdFrontTime(duration)
                }
                if (!isUndefined(ad) && !isUndefined(ad['frontPlay']) && !ad['frontPlay']) {
            ...
            timeUpDate: function() {
                ...
                if (maxSeeTime < oldTime) { maxSeeTime = oldTime }
                ...
                if (!isUndefined(ad) && !isUndefined(ad['frontPlay']) && ad['frontPlay'] && loadedmetadataNum > 1) { calculationAdFrontTime(duration - playTime) }
                if (!isUndefined(C['error']) && C['error'].css('display') == 'block') { C['error'].hide() }
                if (cookieName) { changeCookie(playTime) }
            ...
            ended: function() {
                if (!isUndefined(ad) && !isUndefined(ad['frontPlay']) && ad['frontPlay']) { adFrontEnded() } else {
                    C['bar']['playAndPause']['pause'].hide();
                ...
                if (!isUndefined(event.type)) {
                    ...
                    var errorBak = function() {
                        ...
                        if (!isUndefined(errorInfo)) {
                            if (!isUndefined(errorInfo.code)) { code = errorInfo.code }
                            if (!isUndefined(errorInfo.message)) { msg = errorInfo.message }
                            CT.error = { code: code, message: msg };
                            showError()
                        } else { CT.error = { code: code, message: msg }; if (event.target.attr('src')) { showError() } }
                        eventTarget('error', CT.error)
                    };
                    if (event.type == 'error') {
                        try {
                            if (video.currentSrc) {
                                ajax({
                                    ...
                                    success: function(data) { if (!data) { errorBak() } }
                                })
                            } else { errorBak() }
                        } catch (event) { errorBak() }
                    }
                }
            },
            stalled: function() {
            ...
            suspend: function() { eventTarget('suspend') },
            emptied: function() {
                eventTarget('emptied');
            ...
            seeking: function() {
                ...
                if (firstSeekTime == -1) { firstSeekTime = seekingTime }
            ...
            waiting: function() {
                waited = false;
                eventTarget('buffer', 'start');
                C['centerPlay'].hide();
                if (!vars['controls']) { C['buffer'].show() }
                if (!waitingMessage) { waitingMessage = true } else { message(language['waiting']) }
            },
            play: function() {
                paused = false;
                C['bar']['playAndPause']['play'].hide();
                ...
                C['bar']['playAndPause']['refresh'].hide();
                C['centerPlay'].hide();
                if (playType == 'button') { C['bar']['pbox'].removeClass('ck-bar-progress-out') } else { C['bar']['pbox'].addClass('ck-bar-progress-out') }
                if (recoveryVolume) {
                ...
                closePauseAd();
                playType = '';
                if (!isUndefined(C['ended'])) { player.closeLayer(C['ended']) }
                if ('error' in C) { C['error'].hide() }
                eventTarget('play');
                if (!isUndefined(ad)) {
                    if (!isUndefined(ad['front'])) {
                        if (isUndefined(ad['frontPlay'])) {
                            ad['frontPlayI'] = 0;
                            ad['frontPlay'] = true;
                            player.pause();
                            adFrontPlay();
                ...
                if (seekTime && (isUndefined(vars['live']) || (!isUndefined(vars['live']) && valType(vars['live']) == 'boolean' && !vars['live'])) && seekTime < parseInt(duration * 100) * 0.01) {
            ...
            pause: function() {
                paused = true;
                if (isUndefined(ad) || isUndefined(ad['frontPlay']) || (!isUndefined(ad['frontPlay']) && !ad['frontPlay'])) {
                    if (!isUndefined(ad) && !isUndefined(ad['pause'])) {
                        if (isUndefined(ad['pausePlayI'])) { ad['pausePlayI'] = 0 }
                        adPausePlay()
                    } else {
                        if (!vars['controls']) {
                            C['centerPlay'].show();
            ...
            volumeChange: function() {
                if (this.volume == 0 && !mutedState) { this.muted = true }
                if (this.volume > 0 && mutedState) { this.muted = false }
                if (mutedState != this.muted) {
                    mutedState = this.muted;
                    changeMuted(this.muted);
                    CT.muted = this.muted;
                    eventTarget('muted', CT.muted)
                }
                CT.volume = this.volume;
                eventTarget('volume', this.volume);
                if (!isUndefined(C['bar']['vbox'])) {
                    var vol = parseInt(this.volume * 100);
                    C['bar']['vbox']['volume']['txt'].htm(vol);
                    if (!C['bar']['vbox']['mouseDown']) { changeVolumeSlider(this.muted ? 0 : this.volume) }
                    if (!this.muted) { message(language['volume'] + vol + '%') }
                }
            ...
            resize: function() {
                ...
                if (fullState) {
                    ...
                    if (!CM.hasClass('ck-main-full')) { CM.addClass('ck-main-full') }
                    message(language['full']);
                    C['bar']['webFullAndExit'].hide();
                    C['bar']['theatreAndExit'].hide();
                    if (!isUndefined(C['menu'])) { CM.append(C['menu']) }
                } else {
                    fullState = false;
                    ...
                    if (CM.hasClass('ck-main-full')) { CM.removeClass('ck-main-full') }
                    if (!isUndefined(vars['webFull']) && vars['webFull']) { C['bar']['webFullAndExit'].show() }
                    if (!isUndefined(vars['theatre']) && !CT.webFull) { C['bar']['theatreAndExit'].show() }
                    if (!isUndefined(C['menu'])) { $('body').append(C['menu']) }
                    if (CV.css('width') != CM.css('width')) { player.zoom(100) }
                }
                if (CT.full != fullState) {
                    CT.full = fullState;
                    eventTarget('full', fullState)
                }
                checkVideoRotate();
            ...
            keydown: function(event) {
                ...
                var v = player.volume();
                var pre = function() { if (event.preventDefault) { event.preventDefault() } else { event.returnValue = false } };
                switch (keycode) {
                    case 32:
                        pre();
                        player.playOrPause();
        ...
        var adFrontPlay = function() {
                ...
                if (!isUndefined(ad['front']['list'][ad['frontPlayI']])) {
                    ...
                    if (adv['type'] == 'picture') { adPicturePlay(adv) } else if (adv['type'] == 'node') { adNode(adv) } else {
            ...
            adFrontCloseTime = function() {
                ...
                ad['front']['closeTime']--;
                if (ad['front']['closeTime'] > 0) { ad['front']['closeSetTime'] = setTimeout(adFrontCloseTime, 1000) } else {
                    C['ad']['top']['closeAd'].show();
                    C['ad']['top']['closeTime'].hide()
                }
            },
            calculationAdFrontTime = function(t) {
                if (!isUndefined(t)) { ad['front']['list'][ad['frontPlayI']]['time'] = t }
                ad['front']['countDown'] = 0;
                for (var i = ad['frontPlayI']; i < ad['front']['list'].length; i++) { ad['front']['countDown'] += parseInt(ad['front']['list'][i]['time']) }
            ...
            adFrontEnded = function() {
                if (ad['frontPlayI'] < ad['front']['list'].length - 1) {
                    ...
                    allBarShow();
                    seekTime = vars['seek'];
                    if (ad['front']['video']) { changeVideo(vars['video']) } else { player.play() }
                ...
                var imgLoadHandler = function() {
                    ...
                    img.addListener('load', function() {
                        ...
                        if (w > 0 || h > 0) {
                            ...
                            } else { C['ad']['picture'].append(this) }
                            img.unbind();
                            img.click(function() { eventTarget('frontAdClick', adv) })
                        } else {
                            if (imgloadNum < 10) {
                                imgloadNum++;
                                imgLoadHandler()
            ...
            adNode = function(adv) {
                ...
                ad['front']['node'].click(function() { eventTarget('frontAdClick', adv) });
                calculationAdFrontTime();
                adFrontCountDown();
                C['ad']['bottom']['mutedAndExit'].hide()
            },
            ...
            closeFrontAd = function() { if (!isUndefined(ad) && !isUndefined(ad['front']) && !isUndefined(ad['front']['closeButtonClick'])) { if (valType(ad['front']['closeButtonClick']) == 'function') { try { ad['front']['closeButtonClick']() } catch (event) { console.error(event) } } else if (valType(ad['front']['closeButtonClick']) == 'string') { try { eval(ad['front']['closeButtonClick'] + '()') } catch (event) { console.error(event) } } } else { player.closeFrontAd() } },
            adPausePlay = function() {
                ...
                if (!isUndefined(ad) && !isUndefined(ad['pause']) && !isUndefined(ad['pause']['list']) && ad['pause']['list'].length - 1 >= ad['pausePlayI']) {
                    ...
                    if (!isUndefined(adv['file'])) {
                            ...
                            img.addListener('load', function() {
                                ...
                                if (w > 0 || h > 0) {
                                    ...
                                    if (adv['link']) {
                                        ...
                                        a.append(this);
                                        C['adPause'].append(a)
                                    } else { C['adPause'].append(this) }
                                    C['adPause'].css({ 'width': w + 'px', 'height': h + 'px' });
                                    if (ad['pauseClose']) { C['adPause'].append(ad['pauseClose']) }
                                    img.click(function() { eventTarget('pauseAdClick', adv) })
                                } else {
                                    if (imgloadNum < 10) {
                                        imgloadNum++;
                                        imgLoadHandler()
                        ...
                        if (ad['pauseClose']) { ad['pause']['node'].append(ad['pauseClose']) }
                        ad['pause']['node'].click(function() { eventTarget('pauseAdClick', adv) })
                    }
                    eventTarget('pauseAd', adv);
                    if (!isUndefined(adv['time']) && adv['time'] > 0) {
                        adPauseSetTime = setTimeout(function() {
                            ad['pausePlayI']++;
                            if (ad['pausePlayI'] > ad['pause']['list'].length - 1) { ad['pausePlayI'] = 0 }
                            adPausePlay()
                        }, adv['time'] * 1000)
                    }
                }
        ...
        var player = {
            width: function(value) {
                if (!isUndefined(value)) {
                    if (valType(value) == 'number') { value += 'px' }
                    CT.css({ 'width': value });
                    checkVideoRotate()
                }
                return CK.getWidth()
            ...
            videoWidth: function() { if (video) { return video.videoWidth } return null },
            videoHeight: function() { if (video) { return video.videoHeight } return null },
            zoom: function(fn) {
                if (!isUndefined(fn)) {
                    if (valType(fn) == 'function') { newEvent.addEventListener('zoom', fn) }
                    if (valType(fn) == 'number') {
                        var arr = [C['topBar']['zoomEle']['zoom50'], C['topBar']['zoomEle']['zoom75'], C['topBar']['zoomEle']['zoom100']];
                        for (var i = 0; i < arr.length; i++) { arr[i].removeClass('ck-top-bar-zoom-container-focus') }
                        switch (fn) {
                            case 50:
            ...
            rotate: function(fn) {
                if (!isUndefined(fn)) {
                    if (valType(fn) == 'function') { newEvent.addEventListener('rotate', fn) }
                    if (valType(fn) == 'number') {
                        var arr = [0, 90, 180, 270];
                        var rot = 0;
                        if (arr.length > fn && fn > 0) { rot = arr[fn] } else { if (arr.indexOf(fn) > -1) { rot = fn } }
                        if (rot != nowRotate) {
            ...
            vars: function(obj, val) {
                ...
                var isChange = false;
                for (var k in obj) { if (k in vars && obj[k] != vars[k]) { isChange = true; break } }
                if (isChange) { vars = standardization(vars, obj) }
                if (!isUndefined(obj['video'])) {
                    isChangeDef = true;
                    changeVideo(vars['video'])
                }
                if (!isUndefined(obj['title'])) { C['topBar']['titleEle'].htm(obj['title']) }
            },
            loadstart: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('loadstart', fn) } },
            loadeddata: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('loadeddata', fn) } },
            loadedMetaData: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('loadedMetaData', fn) } },
            canplay: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('canplay', fn) } },
            play: function(fn) { if (valType(fn) == 'object') { if (fn['target'] == C['bar']['playAndPause']['play'] || fn['target'] == C['bar']['playAndPause']['refresh']) { playType = 'button' } } if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('play', fn) } else { if (loadedmetadataNum > 0 && video) { try { video.play() } catch (event) { console.error(event) } } } return player },
            pause: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('pause', fn) } else { if (loadedmetadataNum > 0 && video) { try { video.pause() } catch (event) { console.error(event) } } } return player },
            playOrPause: function() { if (paused) { player.play() } else { player.pause() } return player },
            volume: function(fn) {
                if (!isUndefined(fn)) {
                    if (valType(fn) == 'function') { newEvent.addEventListener('volume', fn) } else if (valType(fn) == 'number' && video) {
                        if (fn < 0) fn = 0;
                        if (fn > 1) fn = 1;
                        if (video.muted) { video.volume = fn * .5 }
                        video.volume = fn
                    }
                } else if (video) { return video.volume }
                return player
            },
            muted: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('muted', fn) } else if (video) { video.muted = true } return player },
            exitMuted: function() { if (video) { video.muted = false; if (video.volume == 0) { player.volume(vars['volume'] > 0 ? vars['volume'] : .8) } } return player },
            time: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('time', fn) } else if (video) { return video.currentTime || 0 } },
            seek: function(fn) {
                if (!isUndefined(fn)) {
                    if (valType(fn) == 'function') { newEvent.addEventListener('seek', fn) }
                    if (valType(fn) == 'number' && video) {
                        switch (vars['timeScheduleAdjust']) {
                            ...
                            case 1:
                                break;
                            case 2:
                                if (fn < oldTime) { message(language['timeScheduleAdjust']['prohibitBackOff']); return }
                                break;
                            case 3:
                                if (fn > oldTime) { message(language['timeScheduleAdjust']['prohibitForward']); return }
                                break;
                            case 4:
                                if (fn < firstSeekTime) {
                                    waitingMessage = false;
            ...
            buffer: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('buffer', fn) } return player },
            ended: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('ended', fn) } return player },
            error: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('error', fn) } return player },
            emptied: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('emptied', fn) } return player },
            duration: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('duration', fn) } else { return duration || 0 } },
            playbackRate: function(fn) {
                if (!isUndefined(fn)) {
                    if (valType(fn) == 'function') { newEvent.addEventListener('playbackRate', fn) } else if (valType(fn) == 'number' && video) {
                        video.playbackRate = fn;
                        CT.playbackRate = fn;
                        eventTarget('playbackRate', fn);
                        changePlaybackrateVal(fn)
                    }
                } else { return video.playbackRate }
            },
            track: function(fn) {
                if (!isUndefined(fn)) {
                    if (valType(fn) == 'function') { newEvent.addEventListener('track', fn) } else if (valType(fn) == 'number' && video) {
                        var track = null;
                        if (!isUndefined(video.textTracks)) { track = video.textTracks }
                        if (track && valType(track) == 'texttracklist') {
                            var arr = vars['track'];
                            for (var i = track.length - 1; i > -1; i--) { video.find('track').eq(i).remove() }
                ...
                } else { return CT.track }
            },
            fastBack: function(num) {
                if (duration == 0) return;
                if (isUndefined(num) || valType(num) != 'number') { num = 20 }
                var time = player.time();
                if (time - num > 0) { time -= num } else { time = 0 }
                player.seek(time);
                return player
            },
            fastNext: function(num) {
                ...
                if (isUndefined(num) || valType(num) != 'number') { num = 20 }
                var time = player.time();
                if (time + num < duration) { time += num } else { time = duration }
                player.seek(time);
                return player
            },
            definition: function(fn) { if (!isUndefined(fn)) { if (valType(fn) == 'function') { newEvent.addEventListener('definition', fn) } if (valType(fn) == 'number') { changeDefinition(fn) } } return player },
            fps: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('fps', fn) } return CT.fps },
            playback: function(fn) { if (!isUndefined(fn)) { if (valType(fn) == 'function') { newEvent.addEventListener('playback', fn) } } else { return playbackTime } return player },
            backLive: function(fn) { if (!isUndefined(fn)) { if (valType(fn) == 'function') { newEvent.addEventListener('backLive', fn) } if (valType(fn) == 'boolean') { if (bool) { C['bar']['backLive'].show() } else { C['bar']['backLive'].hide() } } } return player },
            full: function(fn) {
                if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('full', fn) } else {
                    if (smallWindowsState) { return }
                    if (CT.theatre) { player.exitTheatre() }
                    if (CT.webFull) { player.exitWebFull() }
                    var requestMethod = CK.requestFullScreen || CK.webkitRequestFullScreen || CK.mozRequestFullScreen || CK.oRequestFullscreen || CK.msRequestFullscreen;
                    if (!isUndefined(requestMethod)) { requestMethod.call(CK) } else if (!isUndefined(window.ActiveXObject)) { var wscript = new ActiveXObject('WScript.Shell'); if (wscript !== null) { wscript.SendKeys('{F11}') } } else if (!isUndefined(video.webkitEnterFullscreen) || !isUndefined(video.webkitEnterFullScreen)) {
            ...
            exitFull: function() { var exitFullFun = document.exitFullscreen || document.mozCancelFullScreen || document.webkitExitFullscreen || document.oCancelFullScreen || document.msExitFullscreen; if (!isUndefined(exitFullFun)) { exitFullFun.call(document) } else if (!isUndefined(window.ActiveXObject)) { var wscript = new ActiveXObject('WScript.Shell'); if (wscript !== null) { wscript.SendKeys('{F11}') } } return player },
            fullOrExit: function() { var fullState = document.fullScreen || document.mozFullScreen || document.webkitIsFullScreen || document.msFullscreenElement; if (fullState) { player.exitFull() } else { player.full() } return player },
            webFull: function(fn) {
                if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('webfull', fn) } else {
                    exitSmallWindows();
                    if (!isUndefined(vars['theatre'])) { C['bar']['theatreAndExit'].hide() }
                    CK.css({ 'position': 'fixed', 'top': 0, 'left': 0, 'height': '100%', 'z-index': getMaxZIndex() + 1 });
            ...
            theatre: function(fn) {
                if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('theatre', fn) } else {
                    if (valType(vars['theatre']) == 'array' && vars['theatre'].length == 2) { var fun = vars['theatre'][0]; if (valType(fun) == 'function') { fun() } if (valType(fun) == 'string') { eval(fun + '()') } } else {
                        exitSmallWindows();
                        CK.css({ 'position': 'absolute', 'top': CT.offset()['top'], 'left': 0, 'height': CT.getHeight() + 'px', 'z-index': getMaxZIndex() + 1 });
            ...
            exitTheatre: function() {
                if (valType(vars['theatre']) == 'array' && vars['theatre'].length == 2) { var fun = vars['theatre'][1]; if (valType(fun) == 'function') { fun() } if (valType(fun) == 'string') { eval(fun + '()') } } else {
                    CK.css({ 'position': 'relative' });
                    C['bar']['theatreAndExit']['theatre'].show();
                    C['bar']['theatreAndExit']['exitTheatre'].hide();
                    CK.attr('style', '');
            ...
            smallWindows: function(fn) {
                if (!isUndefined(fn)) {
                    if (valType(fn) == 'function') { newEvent.addEventListener('smallWindows', fn) }
            ...
            loop: function(fn) {
                if (!isUndefined(fn)) {
                    if (valType(fn) == 'function') { newEvent.addEventListener('loop', fn) }
            ...
            screenshot: function(fn) {
                if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('screenshot', fn) } else {
                    if (!isUndefined(vars['screenshot']) && vars['screenshot'] && video) {
                        try {
                            ...
                            var downName = 'video';
                            if (vars['title']) { downName = vars['title'] }
                            C['screenshot']['bar']['down'].attr('href', url).attr('download', downName);
                            return base64
                        } catch (error) { message(language['error']['screenshot'], true) }
                    } else { message(language['screenshotClose'], true) }
                }
                return player
            },
            closeScreenshot: function(bool) { if (!isUndefined(bool) && valType(bool) == 'boolean' && !bool) { C['screenshot'].show() } else { C['screenshot'].hide() } return player },
            layer: function(obj, bar) {
                if (isUndefined(obj)) { return null }
                var cBar = false;
                if (!isUndefined(bar)) { cBar = bar }
                var ele = null;
                if (valType(obj) == 'object') {
                    if ('class' in obj && 'content' in obj) {
                        ele = createlDiv(obj['class']);
                        ele.htm(obj['content'])
                    } else if ('content' in obj) { obj = obj['content'] }
                }
                if (valType(obj) == 'string') {
                    if (obj.substr(0, 1) == '.' || obj.substr(0, 1) == '#') { ele = getElement(obj) } else {
                ...
                if (ele) {
                    ...
                    ele.css({ 'position': 'absolute', 'z-index': zIndex });
                    if (!cBar) { CM.append(ele) } else { C['bar'].append(ele) }
                    ele.CK = CK;
                    ele.show()
                }
                return ele
            ...
            closeLayer: function(ele) {
                if (valType(ele) == 'string') { var temp = $(ele); if (!isUndefined(temp)) { if (temp.length > 0) { ele = temp.eq(0) } } }
                if (!isUndefined(ele) && valType(ele) == 'htmlobject') {
                    if (!isUndefined(ele.captureParentNode)) {
                        var eleP = ele.captureParentNode;
                        if (ele.nextElement) { try { eleP.insertBefore(ele, ele.nextElement) } catch (event) { eleP.append(ele) } } else if (ele.prevElement) { try { eleP.insertAfter(ele, ele.prevElement) } catch (event) { eleP.append(ele) } } else { eleP.append(ele) }
                        ele.css({ 'display': ele.cssDisplay })
                    } else { ele.remove() }
                    if (!isUndefined(ele.cssDisplay)) { ele.css({ 'display': ele.cssDisplay, 'position': ele.cssPosition }) }
                }
                return player
            },
            message: function(str) { message(str); return player },
            ...
            addListener: function(name, fn) { newEvent.addEventListener(name, fn) },
            removeListener: function(name, fn) { newEvent.removeEventListener(name, fn) },
            bar: function(bool) { barShow = bool; if (bool) { C['bar'].show() } else { C['bar'].hide() } },
            rightBar: function(bool) { rightBarShow = bool; if (bool) { C['rightBar'].show() } else { C['rightBar'].hide() } },
            mouseActive: function(fn) { if (!isUndefined(fn) && valType(fn) == 'function') { newEvent.addEventListener('mouseActive', fn) } return this },
            cookie: function(name) {
                if (name == 'delete') { cookie(ckplayerCookie, 'delete', vars['domain']); return null }
                if (!isUndefined(name) && name) { name = name.replace(/[ ]*,[ ]*|[ ]+/g, '').replace(/[ ]*;[ ]*|[ ]+/g, '') }
                var cStr = cookie(ckplayerCookie);
                if (cStr) {
                    var arr = stringToArray(cStr);
                    var tempArr = [];
                    var cTime = parseInt(Date.now() * 0.001);
                    for (i = 0; i < arr.length; i++) { if (parseInt(arr[i][2]) > cTime) { tempArr.push(arr[i]) } }
                    arr = tempArr;
                    if (arr.length > 0) {
                        cookie(ckplayerCookie, 'delete', vars['domain'], vars['cookiePath']);
                        cookie(ckplayerCookie, arrayToString(arr), vars['domain'], vars['cookiePath']);
                        for (var i = 0; i < arr.length; i++) { arr[i] = { name: arr[i][0], time: arr[i][1], expirationTime: date('Y-m-d H:i:s', arr[i][2]), expirationTimeStamp: arr[i][2] }; if (!isUndefined(name) && name == arr[i]['name']) { return arr[i] } }
                        return arr
                    } else { cookie(ckplayerCookie, 'delete', vars['domain'], vars['cookiePath']) }
                }
                return null
            },
            visibilityState: function(fn) {
                if (!isUndefined(fn) && valType(fn) == 'function') {
                    if (!isUndefined(document.visibilityState)) { fn(document.visibilityState === 'visible' ? 'show' : 'hidden') }
            ...
            remove: function() {
                ...
                if (eventTargetList != null) {
                    for (var k in eventTargetList) { eventTemp[k] = eventTargetList[k] }
                ...
                if (frontAdPlay) { this.closeFrontAd() }
                if (pugPlayer) {
                    pugPlayer.pause();
                    pugPlayer.unload();
                    pugPlayer.detachMediaElement();
                ...
                if (!isUndefined(C['menu'])) { C['menu'].remove() }
                if (CT) {
                    CT.unbind();
                    CT.htm('')
                }
        ...
        var checkBar = function() {
                ...
                if (mutedState) { C['bar']['vbox']['muted'].hide() } else { C['bar']['vbox']['exitMuted'].hide() }
                if (!isUndefined(vars['next'])) { C['bar']['nextEpisode'].show() } else { C['bar']['nextEpisode'].hide() }
                if (!isUndefined(vars['webFull']) && vars['webFull'] && !CT.full) { C['bar']['webFullAndExit'].show() } else { C['bar']['webFullAndExit'].hide() }
                if (!isUndefined(vars['theatre']) && !CT.full && !CT.webFull) { C['bar']['theatreAndExit'].show() } else { C['bar']['theatreAndExit'].hide() }
                if (!isUndefined(vars['rightBar']) && vars['rightBar']) { C['rightBar'].show() } else { C['rightBar'].hide() }
                ...
                if (!isUndefined(vars['backLive']) && vars['backLive']) { C['bar']['backLive'].show() }
                if (vars['volume'] == 0) { player.muted() }
            },
            loadLogo = function() {
            ...
            loadMenu = function() {
                if (!isUndefined(vars['menu']) && vars['menu']) { rightMenu = vars['menu'] }
                C['menu'] = createlDiv('ckplayer-ckplayer-menu');
                for (var i = 0; i < rightMenu.length; i++) {
                    var m = rightMenu[i];
                    var li = createlDiv('ck-li');
                    if (!isUndefined(m['underline'])) { li.addClass('ck-underline') }
                ...
                var menuMouseOut = function() {
                    clearTime();
                    menuSetTimeOut = setTimeout(function() { C['menu'].hide() }, 200)
                };
            ...
            aboutHide = function() { C['about'].hide() },
            aboutCopy = function() {
            ...
            getBarHeight = function() { var h = C['bar'].getHeight(); if (parseFloat(C['bar']['pbox'].css('top')) < 0) { h -= parseInt(C['bar']['pbox'].css('top')) } return h },
            hideBar = function() {
                if (mouseSetTime) {
                ...
                mouseSetTime = setTimeout(function() {
                    if (!paused) {
                        ...
                        if (CT.full) { CM.addClass('ck-nocursor') }
                        if (C['rightBar'].css('display') == 'block') { C['rightBar'].addClass('ck-right-bar-hide') }
                        if (C['topBar'].css('display') == 'block') { C['topBar'].addClass('ck-top-bar-hide') }
                    }
                    mouseSetTime = null
                }, vars['barHideTime'])
            },
            ...
            rightBarMouseOut = function() { if (!paused) { hideBar() } },
            ...
            checkPlaybackrate = function() {
                ...
                if (!isUndefined(vars['playbackrateOpen']) && vars['playbackrateOpen']) {
                    ...
                    if (getBarHeight() > C['bar'].getHeight()) { h += (getBarHeight() - C['bar'].getHeight()) }
                    C['bar']['playbackrate']['bgbox'].attr('style', '');
                    C['bar']['playbackrate']['bgbox'].css({ 'width': w + 'px', 'height': (h + 2) + 'px', 'left': -(w - bw) * 0.5 + 'px' });
                    C['bar']['playbackrate']['button'].mouseover(function() {
                        w = C['bar']['playbackrate']['bg'].getWidth();
                        bw = C['bar']['playbackrate']['button'].getWidth();
                        C['bar']['playbackrate']['bgbox'].css({ 'left': -(w - bw) * 0.5 + 'px' })
                    });
            ...
            changePlaybackrateVal = function(num) {
                C['bar']['playbackrate']['bg'].find('button').each(function(index, ele) {
                    ...
                    if (num == val) { ele.addClass('ck-list-p-focus'); if (num != 1) { C['bar']['playbackrate']['button'].htm(ele.htm()) } else { C['bar']['playbackrate']['button'].htm(language['playbackrate']) } } else { ele.removeClass('ck-list-p-focus') }
                })
            },
            checkTrack = function() {
                ...
                for (var i = track.length - 1; i > -1; i--) {
                    ...
                    if (!isUndefined(track[i]['default']) && track[i]['default']) { pbr = i }
                ...
                C['bar']['track']['bgbox'].css({ 'width': w + 'px', 'height': (h + 2) + 'px', 'left': -(w - bw) * 0.5 + 'px' });
                C['bar']['track']['button'].mouseover(function() {
                    w = C['bar']['track']['bg'].getWidth();
                    bw = C['bar']['track']['button'].getWidth();
                    C['bar']['track']['bgbox'].css({ 'left': -(w - bw) * 0.5 + 'px' })
                });
                changeTrackVal(pbr)
            },
            ...
            changeTrackVal = function(num) {
                C['bar']['track']['bg'].find('button').each(function(index, ele) {
                    ...
                    } else { ele.removeClass('ck-list-p-focus') }
                })
            },
            checkDefinition = function() {
                ...
                if (valType(vars['video']) == 'array') {
                    ...
                    for (var i = arr.length - 1; i > -1; i--) {
                        ...
                        if (videoUrl && videoUrl.indexOf(arr[i][0]) > -1) { pi = i }
                    ...
                    if (getBarHeight() > C['bar'].getHeight()) { h += (getBarHeight() - C['bar'].getHeight()) }
                    C['bar']['definition']['bgbox'].attr('style', '');
                    C['bar']['definition']['bgbox'].css({ 'width': w + 'px', 'height': (h + 2) + 'px', 'left': -(w - bw) * 0.5 + 'px' });
                    C['bar']['definition']['button'].mouseover(function() {
                        w = C['bar']['definition']['bg'].getWidth();
                        bw = C['bar']['definition']['button'].getWidth();
                        C['bar']['definition']['bgbox'].css({ 'left': -(w - bw) * 0.5 + 'px' })
                    })
                }
                if (pi > -1) { C['bar']['definition']['bg'].find('button').each(function(index, ele) { var i = parseInt(ele.attr('data-i')); if (pi == i) { ele.addClass('ck-list-p-focus') } else { ele.removeClass('ck-list-p-focus') } }) }
            },
            definitionClick = function(e) {
                C['bar']['definition']['bgbox'].hide();
                setTimeout(function() { C['bar']['definition']['bgbox'].removeCss('display') }, 300);
            ...
            changeDefinition = function(num) {
                C['bar']['definition']['bg'].find('button').each(function(index, ele) {
                    var i = parseInt(ele.attr('data-i'));
                    if (num == i) {
                        if (!isUndefined(video.textTracks) && video.textTracks.length > 0) { for (var y = video.find('track').length - 1; y > -1; y--) { video.find('track').eq(y).remove() } }
            ...
            deletePrompt = function() {
                ...
                if (!isUndefined(C['prompt']) && valType(C['prompt']) == 'array') {
                    for (i = C['prompt'].length; i > -1; i--) { if (!isUndefined(C['prompt'][i])) { C['prompt'][i].remove() } }
                    C['prompt'] = null
                }
                C['prompt'] = []
            },
            changePrompt = function() {
                ...
                for (var i = 0; i < vars['prompt'].length; i++) {
                    ...
                    if (!isUndefined(obj) && 'words' in obj && 'time' in obj && !isUndefined(obj['time']) && !isUndefined(obj['words']) && valType(obj['time']) == 'number') {
                        ...
                        ele.css({ 'left': left + '%' });
                        ele.mouseover(function() { if (this.attr('data-words')) { showPromptWords(this) } });
                        ele.mouseout(function() { if (this.attr('data-words')) { showPromptWords() } });
                        C['prompt'].push(ele)
                    }
                }
            },
            showPromptWords = function(ele) {
                ...
                if (!isUndefined(C['preview'])) { top = CK.getHeight() - parseFloat((getBarHeight() + C['preview']['smallHight'] + parseInt(C['preview'].css('margin-bottom')))) } else {
                ...
                C['promptWords'].css({ 'top': top + 'px', 'left': left + 'px' })
            },
            changePreview = function(preview) {
                ...
                if (!isUndefined(preview)) {
                    ...
                    CM.append(C['preview']);
                    if (isUndefined(vars['preview']['type'])) { vars['preview']['type'] = 0 }
                    ...
                    var loadImg = function(bigW, bigH, smallW, smallH) {
                        if (vars['preview']['type'] == 1) {
                            ...
                            for (var i = 0; i < files.length; i++) {
                                for (var y = 0; y < bigH / smallH; y++) {
                                    ...
                                    img.css({ 'width': bigW + 'px', 'height': smallH + 'px', 'background-position': 'left ' + (-(y * smallH)) + 'px' });
                                    C['preview']['bg'].append(img);
                                    C['preview']['bg']['img'].push(img)
                                }
                            }
                            C['preview']['bg'].css({ 'width': bigW * (bigH / smallH) * (files.length + 1) + 'px', 'height': smallH + 'px' })
                        }
                        ...
                        if (vars['preview']['type'] == 1) {
                            ...
                            C['preview']['frame'].css({ 'width': smallW + 'px', 'height': (smallH - parseFloat(C['preview']['frame'].css('border-width')) * 2) + 'px', })
                        }
                    };
                    if (valType(vars['preview']) == 'object') {
                        if (valType(vars['preview']['file']) == 'array') {
                            ...
                            if (valType(vars['preview']['thumbnail']) == 'array' && vars['preview']['thumbnail'].length > 1) {
                                if (valType(vars['preview']['thumbnail'][0]) == 'number') { thumbnail[0] = vars['preview']['thumbnail'][0] }
                                if (valType(vars['preview']['thumbnail'][1]) == 'number') { thumbnail[1] = vars['preview']['thumbnail'][1] }
            ...
            showPreview = function(time) {
                ...
                if (!isUndefined(C['preview']) && !isUndefined(vars['preview']['thumbnail']) && duration > 0) {
                    ...
                    if (!isUndefined(vars['preview']['scale'])) { scale = vars['preview']['scale'] }
                    var divI = 0,
                        imgI = 0;
                    var left = 0,
                        frameLeft = 0;
                    ...
                    if (C['preview'].css('display') == 'none') { C['preview'].show() }
                    C['preview'].css({ 'height': smallH + 'px', });
                    if (vars['preview']['type'] == 1) {
                        ...
                        var minI = divI - 5,
                            maxI = divI + 5;
                        if (minI < 0) minI = 0;
                        if (maxI > C['preview']['bg']['img'].length - 1) { maxI = C['preview']['bg']['img'].length - 1 }
                        ...
                        left = bgXw - smallW * .5;
                        if (left < 0) { left = 0 }
                        if (left > CK.getWidth() - smallW) { left = CK.getWidth() - smallW }
                        C['preview'].css({ 'width': C['preview']['smallWidth'] + 'px', 'background': 'url(' + files[imgI] + ') no-repeat', 'background-position': '-' + column * smallW + 'px -' + rows * smallH + 'px', 'left': left + 'px' })
                    }
                }
            },
            showError = function() {
            ...
            showNextEpisode = function() {
                ...
                if (valType(vars['next']) == 'object') { if ('content' in vars['next']) { C['next'] = player.layer(vars['next']); return } if ('title' in vars['next']) { tip(C['bar']['nextEpisode'], vars['next']['title']); return } }
                tip(C['bar']['nextEpisode'], language['next'])
            },
            nextClick = function() { if (valType(vars['next']) == 'object') { if ('link' in vars['next']) { window.location.href = vars['next']['link'] } } },
            showEnded = function() {
                if (!CT.loop && !isUndefined(vars['ended'])) {
                    closePauseAd();
                    C['ended'] = player.layer(vars['ended']);
                    if (C['ended']) { C['centerPlay'].hide() }
                    return
                }
            },
            windowScroll = function() {
                if (!CT.smallWindows) return;
                if (CT.webFull || CT.theatre || CT.full) { exitSmallWindows(); return }
                ...
                if (scrollTop > CT.offset()['top'] + CT.getHeight()) {
                    if (!CK.hasClass('ckplayer-ckplayer-smallwindow')) {
                        ...
                        if (vars['smallWindowsDrag']) { drag(true) }
                        allBarHide();
                        checkVideoRotate()
                    }
                } else { exitSmallWindows() }
            },
            exitSmallWindows = function() {
                if (CK.hasClass('ckplayer-ckplayer-smallwindow')) {
                    CK.removeClass('ckplayer-ckplayer-smallwindow');
                    if (vars['smallWindowsDrag']) { drag() }
            ...
            drag = function(state) {
                ...
                var posX = 0,
                    ...
                    posYRecord = 0;
                var open = false;
                if (!isUndefined(state)) { open = state }
                ...
                var docMouseMove = function(e) {
                    ...
                    CK.css({ 'left': left + 'px', 'top': top + 'px' });
                    if (CK.css('position') != 'fixed') { CK.css({ 'position': 'fixed' }) }
                    isDrag = true
                };
                var docMouseUp = function() {
                    doc.removeListener('mousemove', docMouseMove);
                    doc.removeListener('mouseup', docMouseUp);
                    CK.css({ 'cursor': 'default' })
                };
                if (open) { CK.mousedown(mouseDown) } else { CK.unbind('mousedown') }
            },
            checkVideoRotate = function() {
                ...
                var vW = CV.getWidth(),
                    vH = CV.getHeight();
                CV.css({ 'transform': 'rotate(' + nowRotate + 'deg)', '-ms-transform': 'rotate(' + nowRotate + 'deg)', '-moz-transform': 'rotate(' + nowRotate + 'deg)', '-webkit-transform': 'rotate(' + nowRotate + 'deg)', '-o-transform': 'rotate(' + nowRotate + 'deg)', 'width': nowZoom + '%', 'height': nowZoom + '%' });
                if (nowRotate != 0 && nowRotate != 180) { if (vH > ckW && vW > ckH) { if (vH / ckW > vW / ckH) { CV.css({ 'height': nowZoom + '%', 'width': ckH + 'px' }) } else { CV.css({ 'width': nowZoom + '%', 'height': ckW + 'px' }) } return } if (vH < ckW && vW < ckH) { if (ckW > ckH) { CV.css({ 'height': nowZoom + '%', 'width': ckH + 'px' }) } else { if (vH / vW >= ckW / ckH) { CV.css({ 'width': nowZoom + '%', 'height': ckW + 'px' }) } else { CV.css({ 'height': nowZoom + '%', 'width': ckH + 'px' }) } } return } if (vH >= ckW && vW <= ckH) { CV.css({ 'width': nowZoom + '%', 'height': ckW + 'px' }); return } if (vW >= ckH && vH < ckW) { CV.css({ 'height': nowZoom + '%', 'width': ckH + 'px' }) } }
            },
            allBarShow = function() {
                if (barShow) {
                    C['bar'].show();
                ...
                if (rightBarShow) { C['rightBar'].show() }
                C['tempTime'].hide();
                if (!isUndefined(C['logo'])) { C['logo'].show() }
            },
            allBarHide = function(tempTime) {
                if (isUndefined(tempTime)) { tempTime = true }
                C['bar'].hide();
                C['topBar'].hide();
                C['rightBar'].hide();
                if (tempTime) { C['tempTime'].show() }
                if (!isUndefined(C['logo'])) { C['logo'].hide() }
            },
            message = function(str, right) {
                ...
                if (str) {
                    C['message'].htm(str);
                    C['message'].show();
                    if (!C['message'].hasClass('ck-animate')) { C['message'].addClass('ck-animate ck-animate-bouncein') }
                    if (!isUndefined(right) && right) { if (!C['message'].hasClass('ck-message-right')) { C['message'].addClass('ck-message-right') } }
                    if (msgSetTime) {
            ...
            tip = function(ele, str, cl, align) {
                ...
                if (str) {
                    ...
                    C['tip'].show();
                    if (!C['tip'].hasClass('ck-animate')) { C['tip'].addClass('ck-animate ck-animate-bouncein') }
                    if (isUndefined(align)) { align = 'auto' }
                    if (align == 'left') {
                    ...
                    if (isUndefined(closeTipFun)) {
                        ...
                        closeTipMouseOut = function(ele) {
                            ...
                            if (ele) { ele.removeListener('mouseout', closeTipMouseOut) }
                        }
                    }
                    closeTipFun();
            ...
            tipResize = function(ele, cl, align) {
                ...
                if (getBarHeight() > C['bar'].getHeight()) { top -= (getBarHeight() - C['bar'].getHeight()) }
                if (w > ew) { left = offset['left'] - ckOffset['left'] - (w - ew) * .5 } else { left = offset['left'] - ckOffset['left'] + (ew - w) * .5 }
                if (align == 'left') {
                    left = offset['left'] - ckOffset['left'] - w;
                    top = offset['top'] - ckOffset['top']
                }
                if (!isUndefined(cl)) { left = cl['x'] - ckOffset['left'] - w * .5 }
                if (left < 0) { left = 0 }
                if (top < 0) { top = 0 }
                if (left > CM.getWidth() - w) { left = CM.getWidth() - w }
                if (top > CM.getHeight() - h) { top = CM.getHeight() - h }
                C['tip'].css({ 'left': left + 'px', 'top': top - parseInt(C['tip'].css('margin-bottom')) + 'px' })
            },
            progressDragX = function() {
                ...
                var sliderMouseDown = function(e) {
                    e.preventDefault && e.preventDefault();
                    if (!vars['timeScheduleAdjust']) { return }
                    e = e || window.event;
                ...
                var docMouseMove = function(e) {
                    ...
                    if (left < 0) { left = 0 }
                    if (left > bg.getWidth() - slider.getWidth()) { left = bg.getWidth() - slider.getWidth() }
                    ...
                    } else { time = formatLiveTime(left * getLiveTime() / (bg.getWidth() - slider.getWidth())) }
                    tip(slider, time);
                    if (left >= 0) {
                        slider.css({ 'left': left + 'px' });
                        play.css({ 'width': left + slider.getWidth() * .5 + 'px' })
                    } else { pSliderMouseDown = false }
                };
                var docMouseUp = function() {
                    noScrolling();
                    doc.removeListener('mousemove', docMouseMove);
                ...
                var sliderMouseOver = function() {
                    var time = '';
                    if (!vars['live']) { time = formatSeconds(parseInt(slider.css('left')) * duration / (bg.getWidth() - slider.getWidth())) } else { time = language['live'] }
                    isMouseLeave = false;
                    tip(slider, time)
                };
                var sliderMouseLeave = function() { isMouseLeave = true };
                slider.mousedown(sliderMouseDown);
                ...
                var bgMouseDown = function(e) {
                    ...
                    if (!vars['live']) {
                        ...
                        switch (vars['timeScheduleAdjust']) {
                            case 0:
                                ...
                                break;
                            case 2:
                                if (time < oldTime) { message(language['timeScheduleAdjust']['prohibitBackOff']); return }
                                break;
                            case 3:
                                if (time > oldTime) { message(language['timeScheduleAdjust']['prohibitForward']); return }
                                break;
                            case 4:
                                if (time < firstSeekTime) { message(language['timeScheduleAdjust']['prohibitLookBack']); return }
                                break;
                            case 5:
                                if (time > maxSeeTime) { message(language['timeScheduleAdjust']['prohibitForwardNotViewed']); return }
                                break
                        }
                        seekTime = 0;
                        player.seek(time)
                    ...
                    if (w > bg.getWidth() - slider.getWidth() * .5) { w = bg.getWidth() - slider.getWidth() * .5 }
                    if (w < slider.getWidth() * .5) { w = slider.getWidth() * .5 }
                    pSliderMouseDown = true;
                    play.css({ 'width': w + 'px' });
                    slider.css({ 'left': w - slider.getWidth() * .5 + 'px' })
                };
                var moveTimer = null;
                var bgMouseMove = function(e) {
                    ...
                    if (!vars['live']) {
                        ...
                        showPreview(time);
                        time = formatSeconds(time)
                    } else { time = formatLiveTime(getLiveTime() * w / bg.getWidth()) }
                    tip(bg, time, client);
                ...
                var bgMouseOut = function(e) {
                    clearTime();
                    moveTimer = setTimeout(function() { showPreview(-1) }, 30)
                };
            ...
            changeProgress = function(time) {
                if ((valType(vars['live']) == 'boolean' && vars['live'])) { return }
                if (!pSliderMouseDown) {
                    var bg = C['bar']['pbox']['bg'],
                        slider = C['bar']['pbox']['slider'];
                    var playW = (time / duration) * 100;
                    if (valType(vars['live']) == 'number') { playW = '100' }
                    ...
                    if (sliderLeft > 100 - sliderW) { sliderLeft = 100 - sliderW }
            ...
            checkProgressSlider = function() {
                var bg = C['bar']['pbox']['bg'],
                    slider = C['bar']['pbox']['slider'];
                var st = 0;
                if (C['bar']['pbox'].css('transition-duration')) { st = parseFloat(C['bar']['pbox'].css('transition-duration')) * 1000 + 20 }
            ...
            changeLoad = function() {
                if (video) {
                    ...
                    if (len > 0) {
                        ...
                        if (loadTime < bufferEnd) { loadTime = bufferEnd }
            ...
            replaceInformation = function(o, n) {
                if (isUndefined(C['about']['sourceData']) && !isUndefined(C['about'].find('ul'))) {
                    ...
                    while (res = reg.exec(C['about']['sourceData'])) { C['about']['objData'][res[1]] = '0' }
                }
            ...
            calculationFps = function() {
                ...
                if (isUndefined(CT.fps)) { CT.fps = 0 }
                step()
            },
            getElement = function(content) {
                var ele = $(content);
                if (isUndefined(ele)) return;
                if (valType(ele) == 'htmlarray') { ele = ele[0] }
                if (isUndefined(ele.captureParentNode)) {
            ...
            $ = function(obj, eL) {
                ...
                if (!isUndefined(eL)) { parent = eL }
                var res = [];
                if (obj) {
                    if (valType(obj) == 'htmlobject' || valType(obj) == 'object') { res = obj; if (!isUndefined(res.ckplayer)) { return res } } else if (valType(obj) == 'string' && obj != '') {
                        switch (obj.substr(0, 1)) {
                            case '.':
                                obj = obj.substr(1, obj.length);
                                if (parent.getElementsByClassName) { res = parent.getElementsByClassName(obj); if (!res.length) { return null } } else if (!parent && document.getElementsByClassName) { res = document.getElementsByClassName(obj); if (!res.length) { return null } } else { var reg = new RegExp(' ' + obj + ' ', 'i'); var ele = null; if (parent.getElementsByTagName) { ele = parent.getElementsByTagName('*') } else { ele = document.getElementsByTagName('*') } for (var i = 0; i < ele.length; i++) { if (reg.test(' ' + ele[i].className + ' ')) { res.push(ele[i]) } } }
                                if (res) { if (res.length === 0) { res = null } }
                                break;
                            case '#':
                                obj = obj.substr(1, obj.length);
                                if (parent.getElementById) { res = parent.getElementById(obj) } else { res = document.getElementById(obj) }
                                break;
                            default:
                                var reg = new RegExp(' ' + obj + ' ', 'i');
                                if (parent.getElementsByTagName) { res = parent.getElementsByTagName(obj) } else { res = document.getElementsByTagName(obj) }
                                if (res) { if (obj == 'body' || obj == 'document' || obj == 'html') { res = res[0] } else if (res.length === 0) { res = null } } else { res = null }
                                break
                        }
                    } else { res = obj }
                } else { res = document }
                if (res) {
                    if (valType(res) == 'htmlobject' || valType(res) == 'object' || valType(res) == 'document') {
                        ...
                        res.find = function(obj) { return $(obj, this) };
                        res.attr = function(key, value) { if (isUndefined(value)) { return this.getAttribute(key) } else { this.setAttribute(key, value); return this } };
                        res.removeAttr = function(key) { this.removeAttribute(key); return this };
                        res.css = function(key, value) {
                            var keyNew = function(str) {
                                if (str.indexOf('-') != -1) {
                                    ...
                                    var b = '',
                                        c = '';
                                    if (arr.length > 1) { b = arr[1].substr(0, 1).toLocaleUpperCase() + arr[1].substr(1, arr[1].length - 1) }
                                    if (arr.length > 2) { b = arr[2].substr(0, 1).toLocaleUpperCase() + arr[2].substr(1, arr[2].length - 1) }
                                    return a + b + c
                                }
                                return str
                            };
                            if (isUndefined(value)) { if (!isUndefined(key) && valType(key) == 'string') { if (this.currentStyle) { return this.currentStyle[key] } else { return document.defaultView.getComputedStyle(this, null)[key] } } if (isUndefined(key)) { if (this.currentStyle) { return this.currentStyle } else { return document.defaultView.getComputedStyle(this, null) } } }
                            if (valType(key) != 'object') { var newKey = keyNew(key); if (this.length > 1) { for (var i = 0; i < this.length; i++) { this[i].style[newKey] = value } } else { this.style[newKey] = value } } else { for (var item in key) { var objKey = keyNew(item); if (valType(this) == 'htmlarray') { for (var i = 0; i < this.length; i++) { this[i].style[objKey] = key[item] } } else { this.style[objKey] = key[item] } } }
                            return this
                        };
                        res.hasClass = function(cName) { if (isUndefined(cName)) return false; var reg = new RegExp('(\\s|^)' + cName + '(\\s|$)'); if (this.className && this.className.match(reg)) { return true } return false };
                        res.addClass = function(cName) {
                            if (!this.hasClass(cName)) {
                                if (this.className && this.className.substr(this.className.length - 1, 1) != ' ') { this.className += ' ' }
                                this.className += cName
                            };
                            return this
                        };
                        res.removeClass = function(cName) { if (this.hasClass(cName)) { this.className = this.className.replace(new RegExp('(\\s|^)' + cName + '(\\s|$)'), ' '); if (this.className.substr(this.className.length - 1, 1) == ' ') { this.className = this.className.substr(0, this.className.length - 1) } if (!this.className) { this.removeAttribute('class') } }; return this };
                        res.searchClass = function(cName) { var arr = this.className.split(' '); for (var i = 0; i < arr.length; i++) { if (arr[i].substr(0, cName.length) == cName) { return arr[i] } } return this };
                        res.removeCss = function(cName) {
                            var cs = this.attr('style');
                            if (cs) {
                                var arr = cs.split(';');
                                var obj = {};
                                for (var i = 0; i < arr.length; i++) { var arrT = arr[i].split(':'); if (arrT.length == 2 && arrT[0] && arrT[0].trim() != cName) { obj[arrT[0].trim()] = arrT[1].trim() } }
                                this.attr('style', '');
                                this.css(obj)
                            }
                        };
                        res.addListener = function(e, f, t) { return addListener(this, e, f, t) };
                        res.removeListener = function(e, f, t) { return removeListener(this, e, f, t) };
                        res.prev = function() { return prev(this) };
                        res.next = function() { return next(this) };
                        res.append = function(ele) { this.appendChild(ele); return this };
                        res.remove = function() { if (!isUndefined(this.parentNode)) { this.parentNode.removeChild(this) } return null };
                        res.bind = function(e, f, t) { return addListener(this, e, f, t) };
                        res.unbind = function(e, f) { var i = 0; var arr = []; if (!isUndefined(f)) { res.removeListener(e, f) } else if (!isUndefined(e)) { if (!isUndefined(this.listenerList)) { arr = this.listenerList; for (i = 0; i < arr.length; i++) { if (arr[i][0] == e) { res.removeListener(e, arr[i][1]) } } } } else { if (!isUndefined(this.listenerList)) { arr = this.listenerList; for (i = 0; i < arr.length; i++) { res.removeListener(arr[i][0], arr[i][1]) } } } };
                        res.htm = function(val) { if (!isUndefined(val)) { res.innerHTML = val } else { return res.innerHTML } };
                        res.htmReplace = function(ar, val, html) {
                            if (!isUndefined(val) && !isUndefined(ar)) {
                                if (isUndefined(this.attr('data-htm'))) { this.attr('data-htm', html) }
                        ...
                        res.fixed = function() { if (this.css('position') == 'fixed') { return true } if (!isUndefined(this.offsetParent)) { return $(this.offsetParent).fixed() } return false };
                        res.getWidth = function() { return this.offsetWidth };
                        res.getHeight = function() { return this.offsetHeight };
                        if (isUndefined(res.width)) { res.width = function() { return this.offsetWidth } }
                        if (isUndefined(res.height)) { res.height = function() { return this.offsetWidth } }
                        res.resize = function(fn) { addListener(window, 'resize', fn) };
                        res.click = function(fn) { addListener(this, 'click', fn); return this };
                        ...
                        res.mouseover = function(fn) { addListener(this, 'mouseover', fn); return this };
                        res.mouseout = function(fn) { addListener(this, 'mouseout', fn); return this };
                        res.mousedown = function(fn) { addListener(this, 'mousedown', fn); return this };
                        res.mouseup = function(fn) { addListener(this, 'mouseup', fn); return this };
                        res.mousemove = function(fn) { addListener(this, 'mousemove', fn); return this };
                        res.mouseWheel = function(fn) {
                            addListener(this, 'mousewheel', fn);
                            addListener(this, 'DOMMouseScroll', fn, false);
                            return this
                        };
                        res.mouseleave = function(fn) { addListener(this, 'mouseleave', fn); return this };
                        res.touchstart = function(fn) { addListener(this, 'touchstart', fn); return this };
                        res.touchmove = function(fn) { addListener(this, 'touchmove', fn); return this };
                        res.touchend = function(fn) { addListener(this, 'touchend', fn); return this };
                        res.show = function() { this.css('display', 'block'); return this };
                        res.hide = function() { this.css('display', 'none'); return this };
                        res.animate = function(parameter, totalTime, easing, callBack) {
                            if (isUndefined(this.CK)) { return res }
                            var thisTemp = this;
                            var parNode = this.CK;
                            ...
                            var getStartAndEnd = function(arr) {
                                ...
                                switch (arr[0]) {
                                    case 'width':
                                        ...
                                        if (vars.substring(vars.length - 1, vars.length) == '%') { result = parseInt(vars) * h * 0.01 } else { result = parseInt(vars) }
                                        break;
                                    case 'height':
                                        ...
                                        break;
                                    case 'left':
                                        if (!isUndefined(thisTemp.css('left'))) { current = parseInt(thisTemp.css('left')) } else { current = thisTemp.offset()['left'] - parNode.offset()['left'] }
                                        if (vars.substring(vars.length - 1, vars.length) == '%') { result = parseInt(vars) * w * 0.01 } else { result = parseInt(vars) }
                                        thisTemp.css('right', 'auto');
                                        break;
                                    case 'right':
                                        if (!isUndefined(thisTemp.css('right'))) { current = parseInt(thisTemp.css('right')) } else { current = parNode.getWidth() - (thisTemp.offset()['left'] - parNode.offset()['left'] + thisTemp.getWidth()) }
                                        if (vars.substring(vars.length - 1, vars.length) == '%') { result = parseInt(vars) * w * 0.01 } else { result = parseInt(vars) }
                                        thisTemp.css('left', 'auto');
                                        break;
                                    case 'top':
                                        if (!isUndefined(thisTemp.css('top'))) { current = parseInt(thisTemp.css('top')) } else { current = thisTemp.offset()['top'] - parNode.offset()['top'] }
                                        if (vars.substring(vars.length - 1, vars.length) == '%') { result = parseInt(vars) * h * 0.01 } else { result = parseInt(vars) }
                                        thisTemp.css('bottom', 'auto');
                                        break;
                                    case 'bottom':
                                        if (!isUndefined(thisTemp.css('bottom'))) { current = parseInt(thisTemp.css('bottom')) } else { current = parNode.getHeight() - (thisTemp.offset()['top'] - parNode.offset()['top'] + thisTemp.getHeight()) }
                                        if (vars.substring(vars.length - 1, vars.length) == '%') { result = parseInt(vars) * h * 0.01 } else { result = parseInt(vars) }
                                        thisTemp.css('top', 'auto');
                                        break;
                                    case 'alpha':
                                        if (!isUndefined(thisTemp.css('filter')) && thisTemp.css('filter') != 'none') { current = Number(thisTemp.css('filter')) * 100 } else if (!isUndefined(thisTemp.css('opacity')) && thisTemp.css('opacity') != 'none') { current = thisTemp.css('opacity') * 100 } else { current = 100 }
                                        if (vars.substring(vars.length - 1, vars.length) == '%') { result = parseInt(vars) } else { result = vars * 100 }
                                        break
                                }
                                result -= current;
                                if (current != result) { return { 'type': arr[0], 'current': current, 'result': result } }
                                return null
                            };
                            ...
                            for (var i = 0; i < parameterArr.length; i++) { var cssArr = parameterArr[i].split(':'); if (cssArr.length == 2) { var temp = getStartAndEnd(cssArr); if (temp) { newCss.push(temp) } } }
                            var funTime = 0;
                            var timeFun = function() {
                                ...
                                if (thisTemp.tweenPlay) {
                                    ...
                                    } else { stopTween(); if (!isUndefined(callBack)) { callBack(thisTemp) } }
                                }
                            };
                            stopTween();
                        ...
                        res.animatePlay = function() { if (!isUndefined(this.timerTween)) { this.tweenPlay = true } return this };
                        res.animatePause = function() { if (!isUndefined(this.timerTween)) { this.tweenPlay = false } return this }
                    } else {
                        res.each = function(fn) { for (var i in this) { if (this.hasOwnProperty(i)) { if (valType(this[i]) == 'htmlobject') { fn(i, this[i]) } else { try { if (!this[0].nodeName) fn(i, this[i]) } catch (e) { fn(i, this[i]) } } } } };
                        res.eq = function(m) { if (valType(this) == 'htmlarray') { if (this.length >= m + 1) { return this[m] } return null } return null };
                        for (var i = 0; i < res.length; i++) { if ($) { try { res[i] = $(res[i]) } catch (event) {} } }
                    }
                }
                return res
            },
            createlDiv = function(className, html) { var ele = document.createElement('div'); var eleObject = $(ele); if (className) { eleObject.addClass(className) } if (html) { eleObject.htm(html) } return eleObject },
            createlVideo = function() { var ele = document.createElement('video'); var eleObject = $(ele); return eleObject },
            createlButton = function(className, html) {
                var ele = document.createElement('button');
                var eleObject = $(ele);
                eleObject.attr('type', 'button');
                if (className) { eleObject.addClass(className) }
                if (html) { eleObject.htm(html) }
                return eleObject
            },
            ...
            createlJsLink = function(html, js) { var ele = document.createElement('a'); var eleObject = $(ele); if (html) { eleObject.htm(html) } if (!isUndefined(js) && js) { eleObject.attr('href', 'javascript:' + js + '()') } else { eleObject.attr('href', 'javascript:;') } return eleObject },
            createlImg = function(file, className) { var ele = document.createElement('img'); var eleObject = $(ele); if (className) { ele.className = className } if (file) { ele.src = file } return eleObject },
            getClient = function(event) {
                var eve = event || window.event;
                if (isUndefined(eve)) { eve = { clientX: 0, clientY: 0 } }
                ...
                if (isUndefined(x.toString()) || x.toString() == 'NaN') { x = eve.touches[0].clientX }
                if (isUndefined(y.toString()) || y.toString() == 'NaN') { y = eve.touches[0].clientY }
                if (x.toString() == 'NaN') { x = 0 }
                if (y.toString() == 'NaN') { y = 0 }
                return { x: x, y: y }
            },
            getLiveTime = function() { var notTime = Date.now(); var varsTime = vars['live']; if (varsTime.toString().length < 13) { varsTime = varsTime * 1000 } return notTime - varsTime },
            formatLiveTime = function(time) {
                var varsTime = vars['live'];
                if (varsTime.toString().length < 13) { varsTime = varsTime * 1000 }
                varsTime += time;
                return language['lookBack'] + date('H:i:s', varsTime)
            },
            loadJs = function(file, callback) {
                var fn = function() {};
                if (!isUndefined(callback)) { fn = callback }
                if (checkJs(file)) { fn(); return }
                ...
                var doReady = function() {
                    ...
                    if (isReady) return;
                    isReady = true;
                    if (valType(fn) == 'function') { fn() }
                };
                var bodyLoad = function() { timer = setInterval(function() { try { if (!isUndefined(script.readyState) && script.readyState == 'complete') { doReady() } } catch (event) {} }, 10) };
                var w3c = function() {
                    if (valType(fn) == 'function') { fn() }
                ...
                ele.click(function() {
                    if (!setTime) {
                        setTime = setTimeout(function() {
                            try { ele.dispatchEvent(new Event('sigClick')) } catch (event) {
                    ...
                    if (!clickTime) { clickTime = new Date().getTime() } else {
                        if (new Date().getTime() - clickTime < 230) {
                            try { ele.dispatchEvent(new Event('dobClick')) } catch (event) {
                                var e = document.createEvent('HTMLEvents');
            ...
            noScrolling = function(b) { if (isUndefined(b)) b = false; var bodyScroll = function(event) { event.preventDefault() }; if (b) { $('body').addListener('touchmove', bodyScroll, { passive: false }) } else { $('body').removeListener('touchmove', bodyScroll, { passive: false }) } };
        return into(videoObj)
    }

    function ajax(cObj) {
        var callback = null;
        var obj = { method: 'get', dataType: 'json', charset: 'utf-8', async: true, url: '', data: null, success: null, error: null };
        obj = standardization(obj, cObj);
        if (isUndefined(obj['url'])) { return }
        var errorFun = function(info) { if (!isUndefined(obj['error']) && valType(obj['error']) == 'function') { obj['error'](info) } else { obj['success'](null) } };
        var successFun = function(success) { if (!isUndefined(obj['success']) && valType(obj['success']) == 'function') { obj['success'](success) } };
        var createXHR = function() { if (window.XMLHttpRequest) { return new XMLHttpRequest() } else if (window.ActiveXObject) { try { return new ActiveXObject('Microsoft.XMLHTTP') } catch (event) { try { return new ActiveXObject('Msxml2.XMLHTTP') } catch (event) {} } } };
        var formatParams = function(obj) { var arr = []; for (var key in obj) { arr.push(encodeURIComponent(key) + '=' + encodeURIComponent(obj[key])) } return arr.join('&') };
        if (valType(cObj) != 'object') { if (!isUndefined(obj['error'])) { obj.error(event) } return }
        if (obj.dataType === 'json' || obj.dataType === 'text' || obj.dataType === 'html' || obj.dataType === 'xml') {
            var xhr = createXHR();
            callback = function() { if (xhr.status === 200) { if (obj.dataType === 'json') { try { successFun(eval('(' + xhr.responseText + ')')) } catch (event) { successFun(null) } } else { successFun(xhr.responseText) } } else { errorFun({ code: xhr.status, message: xhr.statusText }) } };
            obj.data = formatParams(obj.data);
            if (obj.method === 'get' && !isUndefined(obj.data)) { if (obj.data != '') { if (obj.url.indexOf('?') == -1) { obj.url += '?' + obj.data } else { obj.url += '&' + obj.data } } }
            if (obj.async === true) { addListener(xhr, 'readystatechange', function(event) { if (this.readyState === 4 && callback != null) { callback() } }) }
            ...
            } else { try { xhr.send(null) } catch (event) { callback() } }
            if (obj.async === false) { callback() }
        } else if (obj.dataType === 'jsonp') {
            var oHead = document.getElementsByTagName('head')[0];
            var oScript = document.createElement('script');
            var callbackName = 'callback' + new Date().getTime();
            var params = formatParams(obj.data) + '&callback=' + callbackName;
            oScript.src = obj.url.split('?') + '?' + params;
            oHead.insertBefore(oScript, oHead.firstChild);
            window[callbackName] = function(json) {
                if (!isUndefined(obj['success']) && valType(obj['success']) === 'function') { obj['success'](json) }
    ...
    function documentReady(fn) {
        ...
        var bodyLoad = function() { timer = setInterval(function() { try { if (!isUndefined(document.readyState) && document.readyState == 'complete') { doReady() } } catch (event) {} }, 10) };
        var w3c = function() {
            if (valType(fn) == 'function') { fn() }
            removeListener(window, 'load', w3c)
        };
        if (!isUndefined(document.body)) { if (valType(fn) == 'function') { fn() } } else { try { addListener(window, 'load', w3c) } catch (event) { bodyLoad() } }
    }

    function getPath(siz) {
        var scriptList = document.scripts,
            thisPath = scriptList[scriptList.length - 1].src;
        for (var i = 0; i < scriptList.length; i++) { var scriptName = scriptList[i].getAttribute('name') || scriptList[i].getAttribute('data-name'); var src = scriptList[i].src.slice(scriptList[i].src.lastIndexOf('/') + 1, scriptList[i].src.lastIndexOf('.')); if ((scriptName && (scriptName == 'ckplayer' || scriptName == 'ckplayer.min')) || (scriptList[i].src && (src == 'ckplayer' || src == 'ckplayer.min'))) { thisPath = scriptList[i].src; break } }
        var path = thisPath.substring(0, thisPath.lastIndexOf('/js/') + 1);
        if (!isUndefined(siz)) { path += siz + '/' }
        return path
    }

    function checkJs(file) { var scriptList = document.scripts; for (var i = 0; i < scriptList.length; i++) { if (scriptList[i].src == file) { return scriptList[i] } } return false }

    function getMaxZIndex() { var arr = document.all || document.querySelectorAll('*'); var maxZ = -1; for (var i = 0; i < arr.length; i++) { var temp = null; try { temp = window.getComputedStyle(arr[i], null).zIndex } catch (event) { if (arr[i].style) { temp = arr[i].style.zIndex } } if (temp != 'auto' && parseInt(temp) > maxZ) { maxZ = parseInt(temp) } } return maxZ }

    function getWindowSize() { return { width: window.innerWidth, height: window.innerHeight } }

    function standardization(o, n) { var h = {}; var k; for (k in o) { h[k] = o[k] } for (k in n) { if (k in h) { h[k] = n[k] } } return h }
    ...
    function mergeObj(o, n) {
        ...
        for (k in n) {
            if (k in h) {
                switch (valType(h[k])) {
                    case 'object':
                        if (valType(n[k]) == 'object') { h[k] = mergeObj(h[k], n[k]) }
                        break;
                    default:
                        if (valType(h[k]) == valType(n[k])) { h[k] = n[k] }
                        break
                }
            }
        }
    ...
    function valType(val) { if (typeof val === 'undefined') return 'undefined'; var str = Object.prototype.toString.call(val).split(' ')[1].replace(']', '').toLowerCase(); if (str.substr(0, 4) == 'html' && (str.substr(-7) == 'element' || str.substr(-8) == 'document' || str == 'window')) { str = 'htmlobject' } if (str == 'htmlcollection' || str == 'nodelist') { str = 'htmlarray' } return str }

    function isUndefined(val) { try { return valType(val) === 'undefined' || val === undefined || val === null || (valType(val) === 'number' && isNaN(val)) } catch (event) { return true } return false }

    function prev(ele) { var e = ele.previousSibling; if (e == null) { return null } if (e.nodeType == 3) { var t = prev(e); if (t && t.nodeType == 1) { return t } } else { if (e.nodeType == 1) { return e } else { return false } } }

    function next(ele) { var e = ele.nextSibling; if (e == null) { return null } if (e.nodeType == 3) { var t = next(e); if (t && t.nodeType == 1) { return t } } else { if (e.nodeType == 1) { return e } else { return false } } }

    function addListener(ele, e, f, t) {
        if (isUndefined(t)) { t = false }
        if (ele.addEventListener) { try { ele.addEventListener(e, f, t) } catch (event) {} } else if (ele.attachEvent) { try { ele.attachEvent('on' + e, f) } catch (event) {} } else { ele['on' + e] = f }
        if (isUndefined(ele.listenerList)) { ele.listenerList = [] }
    ...
    function removeListener(ele, e, f, t) {
        if (isUndefined(t)) { t = false }
        if (ele.removeEventListener) { try { ele.removeEventListener(e, f, t) } catch (e) {} } else if (ele.detachEvent) { try { ele.detachEvent('on' + e, f) } catch (e) {} } else { ele['on' + e] = null }
        if (!isUndefined(ele.listenerList)) {
            var temp = [];
            for (var i = 0; i < ele.listenerList.length; i++) { if (ele.listenerList[i][0] != e || ele.listenerList[i][1] != f) { temp.push(ele.listenerList[i]) } }
    ...
    function date(format, timestamp) {
        if (isUndefined(timestamp)) { timestamp = new Date() }
        ...
        var f = {
            ...
            W: function() {
                ...
                if (b <= 2 && ((jsdate.getDay() || 7) - 1) <= 2 - b) { return 1 } else { if (a <= 2 && nd >= 4 && a >= (6 - nd)) { nd2 = new Date(jsdate.getFullYear() - 1 + '/12/31'); return date('W', Math.round(nd2.getTime() / 1000)) } else { return (1 + (nd <= 3 ? ((a + nd) / 7) : (a - (7 - nd)) / 7) >> 0) } }
            },
            F: function() { return txt_months[f.n()] },
            m: function() { return pad(f.n(), 2) },
            M: function() { return f.F().substr(0, 3) },
            n: function() { return jsdate.getMonth() + 1 },
            t: function() { var n; if ((n = jsdate.getMonth() + 1) == 2) { return 28 + f.L() } else { if (n & 1 && n < 8 || !(n & 1) && n > 7) { return 31 } else { return 30 } } },
            L: function() { var y = f.Y(); return (!(y & 3) && (y % 1e2 || !(y % 4e2))) ? 1 : 0 },
            Y: function() { return jsdate.getFullYear() },
            y: function() { return (jsdate.getFullYear() + '').slice(2) },
            a: function() { return jsdate.getHours() > 11 ? 'pm' : 'am' },
            A: function() { return f.a().toUpperCase() },
            B: function() { var off = (jsdate.getTimezoneOffset() + 60) * 60; var theSeconds = (jsdate.getHours() * 3600) + (jsdate.getMinutes() * 60) + jsdate.getSeconds() + off; var beat = Math.floor(theSeconds / 86.4); if (beat > 1000) beat -= 1000; if (beat < 0) beat += 1000; if ((String(beat)).length == 1) beat = '00' + beat; if ((String(beat)).length == 2) beat = '0' + beat; return beat },
        ...
        return format.replace(/([a-zA-Z])/g, function(t, s) { var ret = ''; if (t != s) { ret = s } else if (f[s]) { ret = f[s]() } else { ret = s } return ret })
    }
    ...
    function decodeURIString(obj) { for (var k in obj) { if (valType(obj[k]) == 'string' && obj[k]) { try { obj[k] = decodeURI(obj[k]) } catch (event) {} } if (valType(obj[k]) == 'object') { obj[k] = decodeURIString(obj[k]) } } return obj }

    function getApp() { var u = navigator.userAgent.toLowerCase(); if (u.indexOf('iphone') > -1) { return 'iphone' } }

    function cookie(name, value, domain, path) {
        if (isUndefined(domain)) { domain = '' }
        if (isUndefined(path)) { path = '/' }
        var ckStr = ';domain=' + domain + ';path=' + path;
        if (location.protocol == 'https') { ckStr += ';SameSite=None;Secure=true' }
        var set = function(name, value) {
                var time = 360 * 24 * 60 * 60 * 1000;
                var exp = new Date();
                exp.setTime(exp.getTime() + time);
                try { document.cookie = name + '=' + escape(value) + ';expires=' + exp.toGMTString() + ckStr } catch (event) { console.error(event) }
            },
            get = function(name) { var arr, reg = new RegExp('(^| )' + name + '=([^;]*)(;|$)'); if (arr = document.cookie.match(reg)) { return unescape(arr[2]) } else { return null } },
            del = function(name) {
                var exp = new Date();
                exp.setTime(exp.getTime() - 1);
                var cval = get(name);
                if (cval != null) { document.cookie = name + '=' + cval + ';expires=' + exp.toGMTString() + ckStr }
            };
        if (!isUndefined(name) && !isUndefined(value)) { if (value == 'delete') { del(name); return null } else { set(name, value); return get(name) } } else if (!isUndefined(name) && isUndefined(value)) { return get(name) } else if (!isUndefined(name)) {}
    }

    function arrayToString(arr) { var str = ''; if (!isUndefined(arr)) { for (var i = 0; i < arr.length; i++) { var temp = arr[i]; if (i > 0) { str += ',' } for (var y = 0; y < temp.length; y++) { if (y > 0) { str += ';' } if (valType(temp[y]) == 'number') { str += parseInt(temp[y] * 100) } else { str += temp[y] } } } } return str }
    ...
    function documentHidden(fn) {
        if (!isUndefined(document.visibilityState)) {
            ...
            addListener(document, 'visibilitychange', function() { fn(document.visibilityState === 'visible' ? 'show' : 'hidden') })
        }
    }
