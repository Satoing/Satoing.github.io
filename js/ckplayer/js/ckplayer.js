...
(function(global, factory) {
    ...
    function ckplayerEmbed(videoObj) {
        ...
        var oldTime = 0,
            playTime = 0,
            firstSeekTime = -1,
            maxSeeTime = 0; //oldTime=记录上次播放时间，playTime=当前播放时间，firstSeekTime=记录第一次拖动的时间,maxSeeTime=看过的最大时间
        var isChangeDef = true; //是否需要重置清晰度
        var playType = ''; //播放类别，默认=''，是通过api接口播放，='button'则判定是通过点击按钮播放
        var msgSetTime = null,
            tipSetTime = null,
            mouseSetTime = null; //用于显示提示后自动隐藏的计时器
        var adFrontSetTime = null,
            adPauseSetTime = null; //贴片广告和暂停广告的计时器
        ...
        var ad = null; //广告对象
        var frontAdPlay = false,
            pauseAdPlay = false; //贴片广告是否播放，暂停广告是否播放
        var barShow = true,
            rightBarShow = true;
        var EventTarget = null,
            newEvent = null,
            eventTargetList = null; //注册监听
        var loadedTrack = false; //默认未加载字幕
        var C = {}; //保存播放器界面上的所有节点
        var CT = null,
            CK = null,
            CM = null,
            CV = null; //CT=播放器容器，页面中已存的，CT>CK>CM>CV>video
        ...
        var into = function(obj) {
            ...
            loadFace = function() {
                ...
                C['rightBar']['smallwindows']['open'].click(function() {
                    tip(this, language['smallwindowsClose'], null, 'left');
                    player.smallWindows(false);
                });
                ...
                C['rightBar']['smallwindows']['close'].click(function() {
                    tip(this, language['smallwindowsOpen'], null, 'left');
                    player.smallWindows(true)
                });
                ...
                C['rightBar']['loop']['open'].click(function() {
                    tip(this, language['loopClose'], null, 'left');
                    player.loop(false);
                });
                ...
                C['rightBar']['loop']['close'].click(function() {
                    tip(this, language['loopOpen'], null, 'left');
                    player.loop(true);
                });
            ...
            changeCookie = function(time) {
                ...
                if (!cStr) {
                    ...
                    var have = false;
                    var tempArr = [
                        [cookieName, time, cTime + cookieTime * 3600]
                    ];
        ...
        var checkBar = function() {
            ...
            checkPlaybackrate = function() {
                ...
                if (!isUndefined(vars['playbackrateOpen']) && vars['playbackrateOpen']) {
                    ...
                    var w = C['bar']['playbackrate']['bg'].getWidth(),
                        h = C['bar']['playbackrate']['bg'].getHeight();
            ...
            checkDefinition = function() {
                ...
                if (valType(vars['video']) == 'array') {
                    ...
                    var w = C['bar']['definition']['bg'].getWidth(),
                        h = C['bar']['definition']['bg'].getHeight();
            ...
            showPromptWords = function(ele) {
                ...
                C['promptWords'].show();
                var pW = C['promptWords'].getWidth(),
                    pH = C['promptWords'].getHeight();
            ...
            showPreview = function(time) {
                ...
                if (!isUndefined(C['preview']) && !isUndefined(vars['preview']['thumbnail']) && duration > 0) {
                    ...
                    var scale = 2,
                        thumbnail = vars['preview']['thumbnail'];
                    ...
                    var left = 0,
                        frameLeft = 0;
                    var pW = 0;
                    var smallW = C['preview']['smallWidth'],
                        smallH = C['preview']['smallHight'];
            ...
            checkVideoRotate = function() {
                var ckW = CK.getWidth() * nowZoom * 0.01,
                    ckH = CK.getHeight() * nowZoom * 0.01;
                var vW = CV.getWidth(),
                    vH = CV.getHeight();
                CV.css({
            ...
            volumeDragY = function() {
                ...
                var docMouseMove = function(e) {
                    ...
                    var minTop = bgOffset['top'] - boxOffset['top'],
                        maxTop = bgOffset['top'] + bg.getHeight() - boxOffset['top'] - slider.getHeight();
                    if (sliderTop < minTop) {
                        sliderTop = minTop;
                    }
                    if (sliderTop > maxTop) {
